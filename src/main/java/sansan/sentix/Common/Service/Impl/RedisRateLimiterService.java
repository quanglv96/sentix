package sansan.sentix.Common.Service.Impl;
...
import sansan.sentix.Common.Service.RedisService;
...
public class RedisRateLimiterService {
    ...
    public boolean allowRequest(String uri) {
        ...
        if (ObjectUtils.isEmpty(MAX_REQUESTS)
                ...
                || !MAX_REQUESTS.toString().equalsIgnoreCase(GlobalCache.getConfig(Constants.RATE_LIMIT_MAX_REQ))
                || !WINDOW_SECOND.toString().equalsIgnoreCase(GlobalCache.getConfig(Constants.RATE_LIMIT_WINDOW_SECONDS))) {
            MAX_REQUESTS = Long.valueOf(GlobalCache.getConfig((Constants.RATE_LIMIT_MAX_REQ)));        // Số request tối đa
            WINDOW_SECOND = Long.parseLong(GlobalCache.getConfig((Constants.RATE_LIMIT_WINDOW_SECONDS))); // Trong 10 giây
        }
        if (!checkLimit(ip)) {
